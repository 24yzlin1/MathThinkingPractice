...
def plot_truth_table_matrix(switch_count=3):
    ...
    for switches, light in truth_table:
        ...
        row_colors = ["#ffffff"] * switch_count + [
            "#ff9999" if light == 0 else "#99ff99"
        ]
...
def plot_state_space_graph(switch_count=3, highlight_path=None):
    ...
    if highlight_path:
        path_edges = [
            (tuple(highlight_path[k]), tuple(highlight_path[k + 1]))
            for k in range(len(highlight_path) - 1)
        ]

        nx.draw_networkx_edges(
            G, pos, edgelist=path_edges, width=4, edge_color="blue", alpha=0.8
        )
        nx.draw_networkx_nodes(
            G,
            pos,
            nodelist=[tuple(p) for p in highlight_path],
            node_color="yellow",
            node_size=900,
        )
...
def plot_output_state_distribution(switch_count=3):
    ...
    labels = [
        f"Light OFF (L=0) - {count_off} states",
        f"Light ON (L=1) - {count_on} states",
    ]
...
def plot_correctness_verification_table(switch_count=3):
    ...
    plt.title(
        "Correctness Verification Table\n(Theoretical vs Actual Code Output)",
        fontsize=14,
        pad=20,
    )
