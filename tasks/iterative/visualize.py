...
def plot_error_curves(jacobi_hist, gs_hist=None, tol=1e-8):
    ...
    # 绘制 Jacobi 误差曲线
    plt.semilogy(range(len(jacobi_hist)), jacobi_hist, 
                 marker='o', linestyle='-', markersize=4, label='Jacobi')
                 
    # 绘制 Gauss-Seidel 误差曲线 (如果提供了的话)
    if gs_hist is not None:
        plt.semilogy(range(len(gs_hist)), gs_hist, 
                     marker='s', linestyle='--', markersize=4, label='Gauss-Seidel')
...
def plot_solution_comparison(x_exact, jacobi_x, gs_x=None):
    ...
    # 参考解使用折线图作为基准
    plt.plot(indices, x_exact, 'k-o', linewidth=2, label='参考解 (Direct Solve)', zorder=10)
    
    # Jacobi 数值解使用散点图
    plt.scatter(indices, jacobi_x, marker='s', s=80, color='blue', label='Jacobi 数值解', zorder=5)
    
    # Gauss-Seidel 数值解使用散点图 (如果提供了的话)
    if gs_x is not None:
        plt.scatter(indices, gs_x, marker='^', s=80, color='green', label='G-S 数值解', zorder=5)
